...
function chatAreaInputed() {
    if ($('textarea[id="chat-text"]').val() != '') {
        ...
        $('ul').append('<div class="float-clear"><li class="user">' + chatText + '</li></div>');
    }
}

function apiResponseInputed(text) {
    // Lv.6:ChatBotから返ってきた内容を表示しよう
    $('ul').append('<div class="float-clear"><li class="noby">' + text + '</li></div>');
}
...
function chatbotRequest() {
    ...
    const data = { 'appkey': '5b80aafbec70f108a2aa0bc3595782d0', 'text': $('textarea[id="chat-text"]').val() }
